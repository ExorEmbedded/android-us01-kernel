...
//////////////////////////////////////////////////////////////////
         for (i = 0; i < MAX_FINGER_NUM*2; i++) {
                 if (!point_slot[i].active)
                         continue;
 
                 input_mt_slot(tsdata->input, i);
                 input_mt_report_slot_state(tsdata->input, MT_TOOL_FINGER,
                                 (point_slot[i].active == 2));
 
                 if (point_slot[i].active ==2) {
                         point_slot[i].active = 1;
                         input_report_abs(tsdata->input, ABS_MT_TOUCH_MAJOR,
                                         15);
 ...
 /*
         if (status != MXT_RELEASE) {
                 input_report_abs(tsdata->input, ABS_X, point_slot[single_id].x);
                 input_report_abs(tsdata->input, ABS_Y, point_slot[single_id].y);
                 input_report_abs(tsdata->input,
                                  ABS_PRESSURE, point_slot[single_id].pressure);
         }
 */
         input_sync(tsdata->input);
#endif
					 ...
					 const struct i2c_device_id *id)
{
	...
	input_mt_init_slots(input, MAX_FINGER_NUM * 2); //!!!
