...
struct tsc2004 {
	...
	s16			irq_gpio;
	u16			flags;
	u16			minx, maxx, miny, maxy, deltax, deltay;
...
static int tsc2004_get_pendown_state(struct tsc2004 *tsc)
{
	return !gpio_get_value(tsc->irq_gpio);
}

static int tsc2004_init_platform_hw(struct tsc2004 *tsc)
{
	int ret = 0;

	ret = gpio_request(tsc->irq_gpio, "tsc2004-irq");
	if (ret < 0) {
	...
	if (gpio_direction_input(tsc->irq_gpio)) {
		printk(KERN_WARNING "GPIO#%d cannot be configured as "
				"input\n", tsc->irq_gpio);
		return -ENXIO;
	}

	gpio_set_debounce(tsc->irq_gpio, 0xa);
	return ret;
}

static void tsc2004_exit_platform_hw(struct tsc2004 *tsc)
{
	gpio_free(tsc->irq_gpio);
}
...
static void tsc2004_work(struct work_struct *work)
{
	 ...
	 */
	if (unlikely(!tsc2004_get_pendown_state(ts))) {
	...
	if (rt) {
		...
		if (ts->flags & TSC2004_FLAG_SWAPXY) {
			u16 dummy = tc.x;
			tc.x = tc.y;
			tc.y = dummy;
		}
		if (ts->flags & TSC2004_FLAG_INVERTX)
			tc.x = tc.x ^ MEAS_MASK;
		if (ts->flags & TSC2004_FLAG_INVERTY)
			tc.y = tc.y ^ MEAS_MASK;

		if (ts->use_calibration) {
			if (tc.x < ts->minx) {
				ts->minx = tc.x;
				ts->deltax = ts->maxx - ts->minx + 1;
			} else if (tc.x > ts->maxx) {
				ts->maxx = tc.x;
				ts->deltax = ts->maxx - ts->minx + 1;
			}
			if (tc.y < ts->miny) {
				ts->miny = tc.y;
				ts->deltay = ts->maxy - ts->miny + 1;
			} else if (tc.y > ts->maxy) {
				ts->maxy = tc.y;
				ts->deltay = ts->maxy - ts->miny + 1;
			}
			tc.x = ((tc.x - ts->minx) << 12) / ts->deltax;
			tc.y = ((tc.y - ts->miny) << 12) / ts->deltay;
		}
...
static irqreturn_t tsc2004_irq(int irq, void *handle)
{
	...
	if (likely(tsc2004_get_pendown_state(ts))) {
				   ...
				   const struct i2c_device_id *id)
{
	...
	if ((pdata->irq_gpio <= 0) || (!gpio_is_valid(pdata->irq_gpio))) {
		dev_err(&client->dev, "valid irq_gpio in platform data is required!\n");
		return -EINVAL;
	}
	...
	if ((ts->minx > 0) || (ts->maxx > 0) || (ts->miny > 0) || (ts->maxy > 0))
		ts->use_calibration = true;
	...
	err = tsc2004_init_platform_hw(ts);
	if (err < 0) {
		dev_err(&client->dev, "tsc2004_init_platform_hw failed!\n");
		goto err_free_mem;
	}
